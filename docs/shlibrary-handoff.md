...
|-------|---------|---------|
| *(none)* | `mode=alert` | Alert when book becomes borrowable; empty feed otherwise |
...
- **Next step:** Redeploy Vercel Production if not auto-deployed; re-test `/sub` URLs
- **No open code task** unless Fable's review finds issues or user requests `mode=change`
