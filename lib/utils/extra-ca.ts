import tls from 'node:tls';

import { Agent } from 'undici';

/**
 * ISRG Root YR (Generation Y RSA). Public Let's Encrypt root, not yet in
 * most OS/Node trust stores. latepost.com currently serves
 * EE → YR1 → Root YR without the X1 cross-sign, so Node fetch fails with
 * "unable to get local issuer certificate" / "fetch failed".
 *
 * Source: https://letsencrypt.org/certs/gen-y/root-yr.pem
 */
export const isrgRootYrPem = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`;

export const extraCaList = [...tls.rootCertificates, isrgRootYrPem];

export const extraCaDispatcher = new Agent({
    connect: {
        ca: extraCaList,
    },
});
