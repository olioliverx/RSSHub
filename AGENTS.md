...
RSSHub is a single Node.js/TypeScript service (Hono) that serves RSS/Atom/JSON feeds. Package manager is **pnpm** (via corepack); standard scripts live in `package.json`.

- **Run the app (dev):** `pnpm dev` starts the server on `http://localhost:1200` (health check: `GET /healthz` → `ok`). In dev, routes are loaded dynamically from `lib/routes/`, so no build step is needed just to run the server.
- **Node version:** `engines` wants `^22.20.0 || ^24`. The sandbox's default `node` (on PATH via `/exec-daemon`) is v22.14, so pnpm prints a non-fatal `Unsupported engine` warning — the app, tests, and lint all work anyway. Node 24 is installed via nvm if strict compliance is ever needed (`nvm use 24`), but it does not override the `/exec-daemon` node on PATH.
- **Tests:** `NODE_ENV=test` (used by `pnpm vitest` / `pnpm test`) loads route metadata from `assets/build/routes.json`. This file is generated by `pnpm build:routes` (or `pnpm build`). Run `pnpm build:routes` once before running route/registry/app tests (e.g. `lib/app.test.ts`), or they fail with `Cannot find module '/assets/build/routes.json'`. CI runs `pnpm build` before the suite for this reason. Pure-unit tests (e.g. `lib/utils/*.test.ts`) do not need it.
- **Lint:** `pnpm lint` (ESLint). The repo currently has some pre-existing lint errors/warnings unrelated to environment setup.
- **Puppeteer routes:** browser-automation routes need Chromium (CI runs `pnpm rb && pnpx rebrowser-puppeteer browsers install chrome`). Chromium is NOT installed by default here; most non-Puppeteer routes work without it.
- **Caching/Redis:** cache defaults to in-memory LRU; Redis is optional (`CACHE_TYPE=redis` + `REDIS_URL`). A `Cache not available` log line during `build:routes` is expected and harmless.
